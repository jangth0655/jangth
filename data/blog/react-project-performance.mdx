...
**Note**: 아래 성능 측정 결과는 `yarn build`를 통해 빌드된 클라이언트가 **로컬 서버와 통신**하며 측정한 것입니다. 이 결과는 로컬 환경에서의 성능을 반영하며, 실제 배포 환경에서는 성능이 다를 수 있습니다.
...
결과적으로 `AVIF` 형식과 `sizes` 속성 지정한 경우 파일의 크기는 **약 53.4% 절감**과 로딩시간은 **약 24%로 개선**되었고 **LCP는 5.63초에서 5.25초로 0.4초 개선**된 것을 확인할 수 있다.  
(3G 환경과 성능탭 LCP)
...
`prefetchStores` 함수는 `serverQueryClient`를 반환하는데 `prefetchInfiniteQuery`가 `getStores` 함수로 데이터 패칭**을 수행하고 그 결과를 내부적으로 **QueryClient 캐시에 저장\*\*하기 때문이다.
...
반면 **서버 사이드 데이터 패칭** 경우 로드 시간은 **573ms ~ 1.75s 사이**
...
- 성능탭의 LCP는 **5.65s -> 3.09s** (3G, 캐시 비활성화 환경에서 측정)
- 라이트하우스의 LCP **09s -> 0.6s**

| **항목**             | **개선 전**                                           | **개선 후**                                          |
| -------------------- | ----------------------------------------------------- | ---------------------------------------------------- |
| **이미지 포맷**      | WebP: 29.8 KB ~ 93.1 KB, 1.37s ~ 2.36s                | AVIF: 16.6 KB ~ 63.9 KB, 1.01s ~ 1.76s               |
| **스크립트 로드**    | 모든 스크립트 로드 LCP: 5.25                          | 특정 페이지에서만 스크립트 로드 LCP: 5.09            |
| **데이터 패칭 방식** | 클라이언트 사이드 패칭 (useInfiniteQuery): LCP 5.13초 | 서버 사이드 패칭 (prefetchInfiniteQuery): LCP 3.12초 |
